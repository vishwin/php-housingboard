<nav class="top-bar" data-topbar>
	...
	<section class="top-bar-section">
		...
		<?php if (isset($_SESSION['user'])) { ?><ul class="left">
			...
			<?php if (isset($_GET['id']) && basename($_SERVER['SCRIPT_NAME'])=="post.php" && $db_connection->query('select username from posts where post_id="' . $_GET['id'] . '"')->fetch_assoc()['username']==$_SESSION['user']) { ?><li<?php if (isset($_GET['action']) && $_GET['action']=="modify") { ?> class="active"<?php } ?>><a href="post.php?action=modify&id=<?php echo $_GET['id']; ?>">Edit / delete</a></li><?php } // show edit in navbar if post belongs to logged-in user ?>
		</ul><?php } ?>
	</section>
</nav>
